	...
	if(!empty($_FILES['image']) && $_FILES['image']['size']>0)
	{
			...
			else
			{
				...
				elseif(substr($type,0,5)=='image') 
					...
					$login=mysqli_query($conn,"update users SET f_name='$f_name',l_name='$l_name',profile_pic='$name',city='$city',State='$State',Country='$Country',u_name='$u_name',contact='$contact' where u_name='$email' " ) or die(mysqli_error($conn	)); 
