	...
	if((isset($_GET['cat_id'])))
	{
		...
		if(!empty($_FILES['image']) && $_FILES['image']['size']>0){
			...
			else
			{
				...
				elseif (substr($type,0,5)=='image') {
					...
					$result = $conn->prepare("update category set cat_name='$cat_name',cat_desc='$cat_desc',cat_image='$name' where cat_id='".$_GET['cat_id']."'");
	  //			$result->bind_param("sss",$cat_name,$cat_desc,$name);
		...
		{	  
		    ...
		    $result = $conn->prepare("update category set cat_name='$cat_name',cat_desc='$cat_desc' where cat_id='".$_GET['cat_id']."'");
